...
def _electrostatics_required(f):
    ...
    def wrapper(self, value):
        if value:
            assert (
                USE_WCA
            ), "You can not use electrostatics without a short range repulsive potential. Otherwise oppositely charged particles could come infinitely close."
        return f(self, value)
...
class Properties:
    ...
    def n_nh(self, value):
        if value > self._n_mon:
            raise ValueError(
                "Number of NH monomers cannot be greater than the total number of monomers"
            )
        self._n_nh = value
    ...
    def n_nh2(self, value):
        if value > self._n_mon:
            raise ValueError(
                "Number of NH2 monomers cannot be greater than the total number of monomers"
            )
        self._n_nh2 = value
    ...
    def _set_units(self):
        ...
        self.bjerrum_length = self.ureg.elementary_charge ** 2 / (
            4
            * self.ureg.pi
            * self.ureg.vacuum_permittivity
            * WATER_PERMITTIVITY
            * self.KT
        )
        
        self.ureg.define(
            "sim_energy = {} * boltzmann_constant".format(temperature_kelvin_units)
        )
        ...
        self.box_volume = (
            self.n_poly
            * self.n_mon
            / (self.ureg.avogadro_constant * c_acid_molar_units)
        ).to("nm^3")
        ...
        N_SALT = int((c_salt_molar_units * self.box_volume * self.ureg.avogadro_constant))
        bond_length_angstrom_units = self.bond_l * self.ureg.angstrom
        k_bond_SI_units = self.k_bond * self.KT/self.ureg.angstrom**2
        k_bending_SI_units = self.k_bond_a * self.KT/self.ureg.degree**2
        k_dihedrak_SI_units = self.k_dihedral * self.KT/self.ureg.degree**2

        # Simulation parameters (reduced units)
        self.bond_length_reduced_units = bond_length_angstrom_units.to("sim_length")
        self.k_bond_reduced_units = k_bond_SI_units.to("sim_energy/(sim_length^2)")
        self.k_bending_reduced_units = k_bending_SI_units.to("sim_energy/(radian^2)")
        self.k_dihedral_reduced_units = k_dihedrak_SI_units.to("sim_energy/(radian^2)")
        ...
        for particle, properties in self.input["Particles properties"].items():
            ...
            self.epsilon_reduced_units[particle] = (((properties["epsilon"] * self.ureg.kilojoules/self.ureg.mol)/ self.ureg.avogadro_constant).to("sim_energy")).magnitude
        ...
        print(f"\ttemperature kelvin units: {temperature_kelvin_units:.3f}")
        print(f"\tKT: {self.KT:.3f}")
        print(f"\tc_acid_molar_units: {c_acid_molar_units:.3f}")
        print(f"\tc_salt_molar_units: {c_salt_molar_units:.3f}")

        print(f"\tN monomers: {self.n_acid:.3f}")
        print(f"\tN protonable sites: {self.n_mon:.3f}")
        print(f"\tN_SALT: {N_SALT}\n")

        print(f"\tBond length: {self.bond_length_reduced_units:.3f}")
        print(f"\tBond constant: {self.k_bond_reduced_units:.3f}")
        print(f"\tBending constant: {self.k_bending_reduced_units:.3f}")
        print(f"\tDihedral constant: {self.k_dihedral_reduced_units:.3f}")
        print(f"\tKAPPA: {self.kappa:.3f}")
        print(f"\tKAPPA_REDUCED: {self.kappa_reduced_units:.3f}")
        print(f"\tDebye length: {1. / self.kappa:.2f} = {(1. / self.kappa).to('sim_length'):.2f}")
        print(f"\tBjerrum length: {self.bjerrum_length:.3g} = {self.bjerrum_length.to('sim_length'):.3g}")
        
        print("\nCalculated values:")
        print(f"\tBOX_L: {self.box_length:.3g} = {self.box_length.to('sim_length'):.3g}")
        print(f"\tBOX_V: {self.box_volume:.3g} = {self.box_volume.to('sim_length^3'):.3g}")
        ...
        print(f"{'Particle Type':<{type_width}}{'Sigma (sim length)':<{sigma_width}}{'Epsilon (sim energy)':<{epsilon_width}}{'Charge (sim charge)':<{charge_width}}")

        # Imprime los valores de la tabla
        for particle, properties in self.input["Particles properties"].items():
            charge = (properties["charge"] * self.ureg.e).to("sim_charge").magnitude
            sigma = (properties["sigma"] * self.ureg.angstrom).to("sim_length").magnitude
            epsilon = ((properties["epsilon"] * self.ureg.kilojoules/self.ureg.mol) / self.ureg.avogadro_constant).to("sim_energy").magnitude
            print(f"{particle:<{type_width}}{sigma:<{sigma_width}.3f}{epsilon:<{epsilon_width}.3f}{charge:<{charge_width}.3f}")
    ...
    def _simulation_settings(self):
        ...
        self.sample_iteration_size_capture = 2
        self.sample_begin_capture = 10
        self.n_samp_iter = int(
            (self.num_samples - self.sample_begin_capture)
            / self.sample_iteration_size_capture
        )
        self.number_integration_steps = 500
    ...
    def _read_parameters(self):
        input_ = self.input
        # System properties
        self.temperature = input_["Thermal properties"][
            "Temperature"
        ]  # Temperature (K)
        self.n_poly = input_["System properties"]["Number of polymers"]  # N_Polymers
        self.c_acid = input_["System properties"][
            "Concentration acid particles"
        ]  # C_Polymer (mol/L)
        ...
        self.k_bond = input_["System properties"][
            "bond constant"
        ]  # K_Bond (sim_energy/sim_length^2)
        self.k_bond_a = input_["System properties"][
            "bending constant"
        ]  # K_Bond_a (sim_energy/sim_length^2)
        self.k_dihedral = input_["System properties"][
            "dihedral constant"
        ]  # K_Dihedral (sim_energy/sim_angle^2)
        ...
        self.n_blocks = self.input["Montecarlo properties"][
            "N_BLOCKS"
        ]  # Number of samples per block
        self.block_size = self.input["Montecarlo properties"][
            "DESIRED_BLOCK_SIZE"
        ]  # desired number of samples per block
        self.probability_reaction = self.input["Montecarlo properties"][
            "PROB_REACTION"
        ]  # probability of accepting the reaction move. This parameter changes the speed of convergence.
        self.time_step = self.input["Simulation configuration"][
            "time step"
        ]  # time step (reduced units)

        self.USE_WCA = self.input["Simulation configuration"]["USE_WCA"]
        self.USE_ELECTROSTATICS = self.input["Simulation configuration"][
            "USE_ELECTROSTATICS"
        ]
        ...
        self.USE_DIHEDRAL_POT = self.input["Simulation configuration"][
            "USE_DIHEDRAL_POT"
        ]
