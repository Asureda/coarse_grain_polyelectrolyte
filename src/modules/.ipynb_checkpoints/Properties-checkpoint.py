...
class Properties:
    ...
    def _set_units(self):
        ...
        k_bond_SI_units = (
            self.k_bond * self.ureg.kcal / (self.ureg.mol * (self.ureg.angstrom ** 2))
        )
        k_bending_SI_units = self.k_bond_a * self.ureg.kcal / self.ureg.mol
        k_dihedrak_SI_units = self.k_dihedral * self.ureg.kjoules / self.ureg.mol
        ...
        self.n_salt = int(
            (c_salt_molar_units * self.box_volume * self.ureg.avogadro_constant).to(
                "dimensionless"
            )
        )
        ...
        self.k_bond_reduced_units = (k_bond_SI_units / self.ureg.avogadro_constant).to(
            "sim_energy/(sim_length^2)"
        )
        self.bond_lenth_reduced_units = bond_length_angstrom_units.to("sim_length")
        self.k_bending_reduced_units = (
            k_bending_SI_units / self.ureg.avogadro_constant
        ).to("sim_energy")
        self.k_dihedral_reduced_units = (
            k_dihedrak_SI_units / self.ureg.avogadro_constant
        ).to("sim_energy")
        self.kappa = (
            np.sqrt(c_salt_molar_units.to("mol/L").magnitude) / 0.304 / self.ureg.nm
        )
        self.kappa_reduced_units = self.kappa.to("1/sim_length").magnitude
        # print("bond_lenth_reduced_units",self.bond_lenth_reduced_units)
        # print("k_bond_reduced_units",self.k_bond_reduced_units)
        # print("k_bending_reduced_units",self.k_bending_reduced_units)
        # print("k_dihedral_reduced_units",self.k_dihedral_reduced_units)
        ...
        for particle, properties in self.input["Particles properties"].items():
            ...
            self.charge_reduced_units[particle] = (
                (properties["charge"] * self.ureg.e).to("sim_charge").magnitude
            )
            self.sigma_reduced_units[particle] = (
                (properties["sigma"] * self.ureg.angstrom).to("sim_length").magnitude
            )
